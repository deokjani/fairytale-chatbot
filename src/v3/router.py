...
from src.v3.service import talking, quiz, tts
from src.v3.utils import ebook_lists, get_characters


router_v3 = APIRouter(
    prefix="/v3/book",
    tags=["Book Talking V3 - BERT CEFR with Bad Words Filter"],
)
...
async def _talking(
    ...
    final: bool = Form(
        default=False,
        title="대화 종료",
        description="대화 종료 시 True. BERT로 CEFR을 평가하여 리턴합니다."
    ),
    initial: bool = Form(
        default=False,
        title="초기 인사",
        description="초기 인사말 요청 시 True. A1 수준의 간단한 질문을 합니다."
    ),
):
    return StreamingResponse(talking(book_id, character, session_id, query, final, initial), media_type="text/event-stream")
...
async def _quiz(
    ...
    final: bool = Form(
        default=False,
        title="대화 종료",
        description="대화 종료 시 True. AI가 평가합니다."
    ),
):
